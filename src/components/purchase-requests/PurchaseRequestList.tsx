...
export const PurchaseRequestList: React.FC<PurchaseRequestListProps> = ({ 
  ...
  const handleDelete = async (requestId: string) => {
    const request = filteredAllRequests.find(r => r.id === requestId);
    if (request) {
  ...
  const confirmDelete = async () => {
    if (deleteConfirmation) {
      try {
        await deletePurchaseRequest(deleteConfirmation.requestId);
        // 从选中列表中移除
        if (selectedRequests.includes(deleteConfirmation.requestId)) {
          onSelectionChange?.(selectedRequests.filter(id => id !== deleteConfirmation.requestId));
        }
