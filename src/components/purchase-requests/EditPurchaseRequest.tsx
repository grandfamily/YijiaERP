...
import { X, Plus, Trash2, Save, ZoomIn, Search, ChevronDown } from 'lucide-react';
import { useProcurement } from '../../hooks/useProcurement';
import { useAuth } from '../../hooks/useAuth';
import { PurchaseRequest, PurchaseRequestItem, SKU } from '../../types';
...
const SKUSearch: React.FC<SKUSearchProps> = ({ 
  ...
  useEffect(() => {
    if (!searchTerm.trim()) {
      setFilteredSKUs(skus.slice(0, 10));
    } else {
...
export const EditPurchaseRequest: React.FC<EditPurchaseRequestProps> = ({ 
  request, 
  onClose, 
  onSuccess 
}) => {
  const { updatePurchaseRequest, getSKUs, getSuppliers } = useProcurement();
  const { user } = useAuth();
  ...
  const [items, setItems] = useState<PurchaseRequestItem[]>([...request.items]);
  ...
  const handleItemChange = (index: number, field: string, value: any) => {
    ...
    // Update SKU object if skuId changed
    if (field === 'skuId') {
      const sku = skus.find(s => s.id === value);
      if (sku) {
    ...
    // Update supplier object if supplierId changed
    if (field === 'supplierId') {
      const supplier = suppliers.find(s => s.id === value);
      newItems[index].supplier = supplier;
    }
    ...
    if (field === 'unitPrice' || field === 'quantity') {
      ...
      newItems[index].totalPrice = (newItems[index].unitPrice || 0) * newItems[index].quantity;
    }
