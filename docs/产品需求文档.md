...
### 1.4 系统架构特点
- **事件驱动架构**: 通过自定义事件实现模块间解耦和数据流转
- **状态管理**: 基于观察者模式的Store管理，确保数据一致性
- **权限控制**: 基于角色的细粒度权限管理
- **自动化流转**: 智能的业务流程自动化，减少人工操作
...
**功能描述**: 管理验收不合格的订单
**数据来源**: 
- 到货检验中"半成品待验收"和"成品待验收"验收不合格的SKU
- 自动记录不合格原因、处理时间、处理人员等信息
...
**流出规则**:
- **半成品验收通过** → 流转到"半成品已验收" + 自动同步至"生产排单"的"待排单"
- **成品验收通过** → 流转到"成品已验收" + 自动同步至"统计入库"的"待验收"
- **验收不合格** → 流转到"采购进度"的"不合格订单"子栏目

**联动机制**:
- 采购进度"到货通知"节点批量完成时，自动更新对应SKU的"是否到货"状态
- 验收通过时，自动联动采购进度的"验收确认"节点
...
#### 3.9.2 流转规则
**流入规则**:
- 从"到货检验"的"半成品已验收"自动流转到"待排单"

**流出规则**:
- 生产状态更新为"已完成"时，自动流转到"统计入库"的"待验收"
...
#### 3.10.2 流转规则
**流入规则**:
- 从"到货检验"的"成品已验收"自动流转到"待验收"
- 从"生产排单"的"已完成"自动流转到"待验收"

**流出规则**:
- 验收完成后自动流转到"发货出柜"的"待发货"
...
#### 3.11.2 流转规则
**流入规则**:
- 从"统计入库"的"已验收"自动流转到"待发货"

**流出规则**:
- 确认发货后生成发货批次记录，移动到"已发货"
...
### 5.2 厂家包装流程
```
订单分配 → 采购进度管理 → 纸卡进度管理 → 辅料进度管理 → 财务付款 → 到货检验(成品待验收) → 统计入库 → 发货出柜
```

### 5.3 自己包装流程
```
订单分配 → 采购进度管理 → 纸卡进度管理 → 辅料进度管理 → 到货检验(半成品待验收) → 生产排单 → 统计入库 → 发货出柜
```
...
```typescript
// 半成品验收通过 → 生产排单
if (inspection.productType === 'semi_finished' && qualityResult === 'passed') {
  // 创建生产排单记录并通过事件通知
  window.dispatchEvent(new CustomEvent('addProductionSchedule', { detail: productionScheduleData }));
}

// 成品验收通过 → 统计入库
if (inspection.productType === 'finished' && qualityResult === 'passed') {
  // 创建统计入库记录并通过事件通知
  window.dispatchEvent(new CustomEvent('addQualityControlRecord', { detail: qualityControlRecord }));
}

// 验收不合格 → 采购进度不合格订单
if (qualityResult === 'failed') {
  // 创建不合格订单记录并通过事件通知
  window.dispatchEvent(new CustomEvent('addRejectedOrder', { detail: rejectedOrderData }));
}
```
...
```typescript
// 统计入库验收完成 → 发货出柜
window.dispatchEvent(new CustomEvent('addShippingRecord', { detail: shippingRecord }));
```

#### 5.4.6 事件驱动架构
**自定义事件列表**:
- `addProductionSchedule`: 到货检验 → 生产排单
- `addQualityControlRecord`: 到货检验/生产排单 → 统计入库
- `addRejectedOrder`: 到货检验 → 采购进度不合格订单
- `addShippingRecord`: 统计入库 → 发货出柜
...
```typescript
// 事件派发机制
const dispatchFlowEvent = (eventName: string, data: any) => {
...
// 事件监听机制
useEffect(() => {
  const handleFlowEvent = (event: CustomEvent) => {
  ...
  if (typeof window !== 'undefined') {
    window.addEventListener('eventName', handleFlowEvent as EventListener);
    return () => {
      window.removeEventListener('eventName', handleFlowEvent as EventListener);
    };
  }
}, []);
```
...
#### 15.1.1 自己包装完整流程
```
采购申请 → 审批 → 订单分配(type: in_house) → 
采购进度 + 纸卡进度 + 辅料进度 → 
到货检验(半成品待验收) → 
[验收通过] → 半成品已验收 + 生产排单(待排单) → 
生产排单(已完成) → 统计入库(待验收) → 
统计入库(已验收) → 发货出柜(待发货) → 
发货出柜(已发货)
```
...
#### 15.2.1 财务联动
- 财务管理"定金支付"确认 ↔ 采购进度"定金支付"节点
- 财务管理"付款发货"确认 ↔ 采购进度"尾款支付"节点

#### 15.2.2 纸卡联动
- 纸卡进度所有阶段完成 ↔ 采购进度"纸卡提供"节点

#### 15.2.3 验收联动
- 到货检验"验收通过" ↔ 采购进度"验收确认"节点
